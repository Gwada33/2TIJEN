...
function ProductCard(props: {
  ...
  return (
    <article className="flex flex-col border border-line bg-surface/40" aria-labelledby={`p-${d.id}`}>
      {/* Bandeau : numéro du design + état du stock */}
      <div className="flex items-center justify-between border-b border-line px-4 py-2.5 text-[0.65rem] font-bold uppercase tracking-[0.2em]">
      ...
      <button
        ...
        className="card-tee relative block aspect-[5/4] w-full cursor-pointer bg-night text-left"
      ...
      >
        <span className="flip-stage absolute inset-0 block p-3">
        ...
        <span className="absolute bottom-3 left-4 flex gap-1.5" aria-hidden="true">
        ...
        <span className="absolute bottom-2 right-3 grid h-9 w-9 place-items-center border border-line bg-night/70 text-base backdrop-blur" aria-hidden="true">↻</span>
      </button>
      {/* filet madras aux couleurs du design */}
      <div className="madras-chip h-1.5" style={madras} aria-hidden="true" />
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        ...
        {canBuy && sold > 0 && !allSoldOut && (
          <div className="mt-4 h-1 bg-line" role="presentation">
        ...
        <div className={`mt-5 ${needSize ? "shake outline outline-1 outline-offset-4 outline-orange" : ""}`} role="group" aria-label={`Choisir une taille, ${d.name}`}>
